...
| `run_shell` | Allowlisted shell binary inside a resource folder (on Windows it spawns through a shell so `.cmd` shims — npm/npx/vite — run, and rejects shell metacharacters in args) | `readonly` + shell allowlist + sandboxed `cwd` |
