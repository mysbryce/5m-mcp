...
description: The 16 defence layers that keep one mistake from being catastrophic.
---
...
| 6 — Readonly mode | `agent_api_readonly` (defaults to `true`) blocks every mutating tool | Production lock-down; opt into writes with `false` |
...
| 8 — Extension allowlist | broad text-file set in `sandbox.ts` (read shares the write set) | Writing/reading `.exe`, `.dll`, binary blobs |
