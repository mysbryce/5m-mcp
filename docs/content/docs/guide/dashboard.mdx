...
- **Console** — **realtime** server console (incremental ~1.5s polling). FiveM `^0`–`^9` caret colors are parsed (txAdmin-style) with the color reset at end of each line, each resource/channel prefix gets its own stable hashed color, and prefixes align in a fixed-width column. Toggle **Live** off to freeze it.
