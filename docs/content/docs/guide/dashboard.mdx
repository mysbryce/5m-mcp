...
## Logs tab (master only)

Two read-only views into what the agent has been doing, served from the same handler:

- **Console** — the most recent lines from the in-memory console ring buffer (the same buffer `tail_console` reads), so you can watch resource output and errors live.
- **Audit** — the audit trail of every agent tool call: timestamp, tool name, result code, and the (hashed) caller token. Backed by `dist/audit.log`.

Hit **Refresh** to pull the latest. This is the human-facing counterpart to the `tail_console` / `scan_errors` tools and the `agent://console` MCP resource.
